...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		if (!this.isEmpty()) {
			...
			if (left < this.size() && this.comparator.compare(this.heap[left], this.heap[largest]) > 0) {
				largest = left;
			} 

			if (right < this.size() && this.comparator.compare(this.heap[right], this.heap[largest]) > 0) {
				largest = right;
			}
